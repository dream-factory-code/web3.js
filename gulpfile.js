...
var packages = [
    ...
    },
    {
        fileName: "web3-tolar",
        expose: "Tolar",
        src: "./packages/web3-tolar/dist/index.js",
    },
    {
...
packages.forEach(function (pckg, i) {
    ...
    gulp.task(
        ...
        gulp.series(prevPckg, function () {
            ...
            stream = bundle.pipe(
                exorcist(path.join(DEST, pckg.fileName + ".js.map"))
            );

            if (pckg.fileName === "web3") {
                bundle.pipe(
                    exorcist(
                        path.join(WEB3_PACKAGE_DEST, pckg.fileName + ".js.map")
                    )
                );
            }
            ...
            if (pckg.fileName === "web3") {
                stream = stream.pipe(gulp.dest(WEB3_PACKAGE_DEST));
            }
...
gulp.task(
    ...
    gulp.series(
        ...
        "lint",
        "clean",
        packages[packages.length - 1].fileName
    )
);

gulp.task(
    "default",
    gulp.series("version", "lint", "clean", packages[0].fileName)
);
