...
Accounts.prototype.create = function create(entropy) {
    return this._addAccountFunctions(
        Account.create(entropy || utils.randomHex(32))
    );
};
...
Accounts.prototype.signTransaction = function signTransaction(
    ...
    var _this = this,
        ...
        hasTxSigningOptions = !!(
            tx &&
            ((tx.chain && tx.hardfork) || tx.common)
        );
    ...
    function signed(tx) {
        ...
        try {
            var transaction = helpers.formatters.inputCallFormatter(
                _.clone(tx)
            );
            ...
            if (!hasTxSigningOptions) {
                ...
                if (transaction.common) {
                    transactionOptions.common = Common.forCustomChain(
                        ...
                        {
                            name:
                                transaction.common.customChain.name ||
                                "custom-network",
            ...
            var result = {
                messageHash:
                    "0x" + Buffer.from(ethTx.hash(false)).toString("hex"),
    ...
    return Promise.all([
        ...
        isNot(tx.nonce)
            ? _this._ethereumCall.getTransactionCount(
                  _this.privateKeyToAccount(privateKey).address
              )
            : tx.nonce,
...
Accounts.prototype.recoverTransaction = function recoverTransaction(rawTx) {
    ...
    var extraData =
        recovery < 35
            ? []
            : [Bytes.fromNumber((recovery - 35) >> 1), "0x", "0x"];
...
Accounts.prototype.hashMessage = function hashMessage(data) {
    ...
    var ethMessage = messageBuffer; //Buffer.concat([preambleBuffer, messageBuffer]);
    return Hash.keccak256s(ethMessage);
};

Accounts.prototype.sign = function sign(data, privateKey) {
    console.log("accounts sign called");
    if (!privateKey.startsWith("0x")) {
        privateKey = "0x" + privateKey;
    }
    ...
    var hash = data; //this.hashMessage(data);
...
Accounts.prototype.decrypt = function (v3Keystore, password, nonStrict) {
    ...
    var mac = utils
        .sha3(Buffer.concat([derivedKey.slice(16, 32), ciphertext]))
        .replace("0x", "");
    if (mac !== json.crypto.mac) {
        throw new Error("Key derivation failed - possibly wrong password");
    }

    var decipher = cryp.createDecipheriv(
        json.crypto.cipher,
...
Accounts.prototype.encrypt = function (privateKey, password, options) {
    ...
    var cipher = cryp.createCipheriv(
